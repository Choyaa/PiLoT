...
class DualProcessTask:        
    ...
    def rendering_worker(self):
        ...
        win_y = 0
        window_name = 'Real-time Rendering (Right Half)'
        # 3. 使用 cv2.WINDOW_NORMAL 创建窗口
        #    注意：必须先创建窗口，然后才能移动和调整它
        ...
        # ⭐ 新增：轨迹窗口
        # traj_window_name = "TrajectoryView"
        # cv2.namedWindow(traj_window_name, cv2.WINDOW_NORMAL)
        # # 轨迹窗口大小（你可调成你想要的大小）
        # cv2.resizeWindow(traj_window_name, win_width,  win_height//2)
        # # 屏幕左下角
        # cv2.moveWindow(traj_window_name, 0, screen_height - win_height//2)
        # demo_image = np.zeros((win_height, win_width, 3), dtype=np.uint8)
        # cv2.putText(demo_image, 'Right Half Screen', (50, 100), cv2.FONT_HERSHEY_SIMPLEX, 2, (255, 255, 255), 3)
        while True:
            try:
            ...
            qx0 = x_off
            qy0 = y_off
            color_crop = color[qy0:qy0+crop_h, qx0:qx0+crop_w]
            if query_img is not None:
            # 叠加 query_crop 到 ref
                query_img = cv2.cvtColor(query_img, cv2.COLOR_RGB2BGR)
                frame = query_img.copy()
                frame[y_off:y_off+crop_h, x_off:x_off+crop_w] = color_crop
                font = cv2.FONT_HERSHEY_SIMPLEX
                if fps<25: fps=25
                label = 'PiLoT: ' + str(fps) +' fps'
                cv2.putText(frame, label, (10, 25), font, 0.5, (255, 255, 255), 2, cv2.LINE_AA)
                
                # 把轨迹图贴到 frame 右上角
                roll, pitch, yaw = euler
                lon, lat, alt = trans      # 如果 trans 不是 LLA，我帮你转
                traj_img = self.traj.update(lon, lat, alt, roll, pitch, yaw)
                merged = vstack_images(frame, traj_img)
                cv2.imshow(window_name, merged)
                # # 展示轨迹窗口
                # cv2.imshow(traj_window_name, traj_img)
                # # 展示主窗口
                # cv2.imshow(window_name, frame)
                
                if cv2.waitKey(1) & 0xFF == ord('q'):
                    self.stop_evt.set()  # 通知其他进程也该停了
                    break
            
            
            # print('render:', os.path.join(self.outputs, qname))
            # np.save(os.path.join(self.outputs, str(fps_log_every-2)+'_1.png'), depth)
    ...
    #         if item is None:                          # 对称哨兵
    #             break
    #         euler, trans,qname = item
    #         # ---- 1) 测渲染耗时 ----
    #         t0 = time.perf_counter()
            
    #         with torch.cuda.stream(render_stream):
    #             for _ in range(10):  # 如果需要 Near/Far 收敛
    #                 renderer.update_pose(trans, euler)
    #             color = renderer.get_color_image()
    #             depth = renderer.get_depth_image()
    #         # renderer.save_color_image(os.path.join(self.outputs, str(fps_log_every-2)+'_0.png'))
    #         # renderer.save_color_image(os.path.join(self.outputs, qname))
    #         color_np_rgb = (color.cpu().numpy() * 255).astype(np.uint8) 
    #         color_np_bgr = cv2.cvtColor(color_np_rgb, cv2.COLOR_RGB2BGR)
    #         cv2.imshow('Real-time Rendering', color_np_bgr)
    #         if cv2.waitKey(1) & 0xFF == ord('q'):
    #             break
...
if __name__ == "__main__":
    ...
    # name = 'DJI_20251118180014_0002_V'
