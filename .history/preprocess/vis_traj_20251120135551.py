...
class TrajectoryDrawer:
    def __init__(self, lon0, lat0, alt0, 
                 ...
                 min_view_range_m=300):  # 默认最小视野100米
    ...
    def _normalize_to_canvas(self, x, y, x_min, x_max, y_min, y_max):
        ...
        if padded_range == 0:
            padded_range = 1
        # 4. 计算以轨迹中心为中心、包含留白的新的“世界边界”
        x_center = (x_max + x_min) / 2
        y_center = (y_max + y_min) / 2
        
        map_min_x = x_center - padded_range / 2
        map_min_y = y_center - padded_range / 2
        # 5. 执行映射
    ...
    def update(self, lon, lat, alt, roll, pitch, yaw):
        """这个方法无需修改，它会自动使用新的 _normalize_to_canvas 逻辑"""
        ...
        # 获取 FOV 的所有点, 也加入到边界计算中, 确保 FOV 不会被切掉
