...
def get_3D_samples_v3(mkpts_r, depth_mat, T_c2w, camera, query_euler_angles, query_translation, last_frame_info = {}, origin = None, device = 'cuda', num_init_pose = 32, T_query_in_ECEF_c2w = None, mul = None):
    ...
    max_offset_angle = 0.0
    min_offset_translation = 0.0
    max_offset_translation = 3.0 # 0.015, 0.025 
    # preprocess
...
def get_3D_samples_v4(mkpts_r, depth_mat, T_c2w, camera,  query_euler_angles, query_translation, last_frame_info = {}, origin = None, device = 'cuda', num_init_pose = 32, T_query_in_ECEF_c2w = None, mul = None):
    '''
    1. 欧拉角转R， 经纬高转xyz
    2. preprocess_param， [:3, 1] = -[:3, 1], [:3, 2] = - [:3, 2]
    3. * mul
    ...
    max_pitch=15.0, pitch_step=2.0,
    max_yaw=15.0, yaw_step=2.0,
    max_roll=1.0, roll_step=1.0
    )
    ...
    base_trans=query_translation,
    max_x=9, step_x=3,  # 示例: X轴左右各撒0.5，步长0.1
    max_y=9, step_y=3,  # Y轴
    max_z=0, step_z=0,  # Z轴
    ...
    # ================= 4.以此计算矩阵 =================
    query_T_c2w = euler_angles_to_matrix_ECEF_batch_special_cuda(
