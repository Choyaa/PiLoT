...
def orthogonalize_rotation_matrix(R):
    ...
    R_orth = np.dot(U, Vt)
    return R_orth
def ECEF_to_WGS84_batch(ecef_coords):
    """
    一个简化的、支持批处理的 ECEF 到 WGS84 (经纬高) 转换函数。
    注意：这是一个简化示例，实际应用中应使用更精确的库，如 pyproj。
    输入: ecef_coords - (n, 3) 的 numpy 数组，列为 [x, y, z]
    输出: (n, 3) 的 numpy 数组，列为 [lon, lat, alt] (单位: 度, 度, 米)
    """
    x, y, z = ecef_coords[:, 0], ecef_coords[:, 1], ecef_coords[:, 2]
    
    # WGS84 椭球参数
    a = 6378137.0  # 赤道半径
    e_sq = 6.69437999014e-3  # 扁率的平方
    p = np.sqrt(x**2 + y**2)
    lon = np.arctan2(y, x)
    
    # 迭代法计算纬度和高度 (Bowring's formula)
    lat = np.arctan2(z, p * (1 - e_sq))
    # 为简化，这里只迭代一次，实际需要循环或更精确算法
...
def get_rotation_enu_in_ecef_batch(lon, lat):
    """
    根据经纬度，批量计算从 ENU (East-North-Up) 到 ECEF 的旋转矩阵。
    输入:
        lon: (n,) 数组，经度 (单位: 度)
        lat: (n,) 数组，纬度 (单位: 度)
    输出:
...
# 2. 转换后的批处理主函数
# ===================================================================
def pixloc_to_osg_batch(T_batch_c2w):
    """
    将一批 (n, 4, 4) 的相机到世界(ECEF)的变换矩阵，转换为WGS84坐标和ENU姿态。
    参数:
    T_batch_c2w (np.ndarray): 输入的变换矩阵，形状为 (n, 4, 4)。
    返回:
    tuple: (lons, lats, euler_angles_batch)
        - lons (np.ndarray): 经度数组，形状为 (n,)
        - lats (np.ndarray): 纬度数组，形状为 (n,)
        - euler_angles_batch (np.ndarray): ENU坐标系下的欧拉角(xyz)，形状为 (n, 3)
    """
    # 确保输入是 numpy 数组
    T_batch_c2w = np.asarray(T_batch_c2w)
    
    # 1. 批量提取旋转 R 和平移 t
    # 输入: (n, 4, 4) -> 输出: R(n, 3, 3), t(n, 3)
    R_batch_c2w = T_batch_c2w[:, :3, :3]
    t_batch_c2w = T_batch_c2w[:, :3, 3]
    # 2. 批量将 ECEF 坐标转换为 WGS84 经纬度
    # 输入: (n, 3) -> 输出: (n, 3)
    t_batch_wgs84 = ECEF_to_WGS84_batch(t_batch_c2w)
    lons, lats, _ = t_batch_wgs84[:, 0], t_batch_wgs84[:, 1], t_batch_wgs84[:, 2]
    # 3. 批量计算 ENU 到 ECEF 的旋转矩阵
    # 输入: (n,), (n,) -> 输出: (n, 3, 3)
    rot_enu_in_ecef_batch = get_rotation_enu_in_ecef_batch(lons, lats)
    # 4. 批量求转置，得到 ECEF 到 ENU 的旋转
    # 使用 np.transpose 并指定轴的顺序 (0, 2, 1) 来转置最后两个维度
    # 输入: (n, 3, 3) -> 输出: (n, 3, 3)
    rot_ecef_in_enu_batch = np.transpose(rot_enu_in_ecef_batch, (0, 2, 1))
    # 5. 批量将相机的 ECEF 姿态转换为 ENU 姿态
    # np.matmul/@ 会自动处理批处理矩阵乘法
    # 输入: (n, 3, 3), (n, 3, 3) -> 输出: (n, 3, 3)
    rot_pose_in_enu_batch = rot_ecef_in_enu_batch @ R_batch_c2w
    # 6. 批量从旋转矩阵计算欧拉角
    # R.from_matrix 和 .as_euler 都支持批处理
    # 输入: (n, 3, 3) -> 输出: (n, 3)
    rot_pose_in_enu_obj = R.from_matrix(rot_pose_in_enu_batch)
    euler_angles_batch = rot_pose_in_enu_obj.as_euler('xyz', degrees=True)
    return lons, lats, euler_angles_batch
...
def get_sorted_image_paths_uavscenes(directory_path, extension=".jpg"):
    """
    获取指定目录中所有图像文件的完整路径，并按文件名中的时间戳进行数字排序。
    Args:
        directory_path (str): 包含图像文件的目录路径。
        extension (str): 文件扩展名，默认为 ".jpg"。
    Returns:
        list: 按时间戳升序排列的文件完整路径列表。
              如果找不到文件，则返回一个空列表。
    """
    # 构造一个匹配模式，例如: /your/path/*.jpg
    # glob.glob 会自动找到所有匹配该模式的文件路径
    search_pattern = os.path.join(directory_path, f"*{extension}")
    image_paths = glob.glob(search_pattern)
    if not image_paths:
        print(f"警告：在目录 '{directory_path}' 中未找到任何 '{extension}' 文件。")
        return []
    # --- 核心排序逻辑 ---
    # 使用 sorted() 函数的 key 参数来自定义排序规则
    # 1. os.path.basename(path): 从完整路径中获取文件名，例如 '1698219309.899845123.jpg'
    # 2. os.path.splitext(basename)[0]: 分离文件名和扩展名，并取第一部分，得到 '1698219309.899845123'
    # 3. float(...): 将字符串转换为浮点数
    # lambda path: ... 是一个匿名函数，它对列表中的每个元素（路径）执行这些操作，
    # sorted() 函数会使用这个函数的返回值（浮点数）作为排序的依据。
    sorted_paths = sorted(
        image_paths, 
