...
from ..utils.transform import kf_predictor, pixloc_to_osg, pixloc_to_osg_batch, move_inputs_to_cuda, visualize_points_on_images
...
class BaseRefiner:
    ...
    def refine_query_pose(self, qname: str, qcamera: Camera, ref_camera: Camera, render_frame, T_query_initial: Pose, T_render: Pose, Points_3D_ECEF, dd = None,
        ...
        if ret['success']:
            ...
            euler_angles_candidates = pixloc_to_osg_batch(T_opt_c2w)
            translation_candidates = T_opt_c2w[:, :3, 3].cpu().numpy()
            gt_euler = gt_pose_dict[qname.split('/')[-1]]['euler']
            gt_trans = gt_pose_dict[qname.split('/')[-1]]['trans']
            
            gt_trans = np.array(gt_trans)
            gt_euler = np.arrray(gt_euler)
            positional_errors = np.linalg.norm(translation_candidates - gt_trans, axis=1)
            rotational_errors = np.linalg.norm(euler_angles_candidates - gt_euler, axis=1)
            
            for ii in range(len(euler_angles_candidates)):
                print(positional_errors[ii], rotational_errors[ii])
